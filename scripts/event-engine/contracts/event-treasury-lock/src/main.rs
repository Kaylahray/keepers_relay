...
fn validate() -> Result<(), i8> {
    ...
    if args.len() != TREASURY_ARGS_LEN { return Err(ERR); }
    ...
    if gi == 0 && go == 1 {
        ...
        if event_out.len() != EVENT_DATA_LEN || event_out[EVENT_OFFSET_STATUS] != STATUS_REGISTRATION { return Err(ERR); }
        if bytes::<32>(&event_out, EVENT_OFFSET_TREASURY_LOCK)? != my_lock_hash { return Err(ERR); }
        let (count, spendable) = spendable_pot(&my_lock_hash, Source::Output)?;
        if count != 1 || spendable != 0 { return Err(ERR); }
        return Ok(());
    }
    ...
    if gi == 1 && go == 1 {
        ...
        let event_out = find_event_data(&event_type_hash, &event_id, Source::Output)?;
        if event_in.len() != EVENT_DATA_LEN || event_out.len() != EVENT_DATA_LEN { return Err(ERR); }
        let a = event_in[EVENT_OFFSET_STATUS];
        let b = event_out[EVENT_OFFSET_STATUS];
        if a >= STATUS_SETTLED || b == STATUS_SETTLED { return Err(ERR); }
        if bytes::<32>(&event_out, EVENT_OFFSET_TREASURY_LOCK)? != my_lock_hash { return Err(ERR); }
        let (ic, in_pot) = spendable_pot(&my_lock_hash, Source::Input)?;
        let (oc, out_pot) = spendable_pot(&my_lock_hash, Source::Output)?;
        if ic != 1 || oc != 1 || out_pot < in_pot { return Err(ERR); }
        if a >= STATUS_FINISHED && out_pot != in_pot { return Err(ERR); }
        if read_u64(&event_out, EVENT_OFFSET_POT)? != out_pot { return Err(ERR); }
        return Ok(());
    }
    ...
    if gi == 1 && go == 0 {
        ...
        let event_out = find_event_data(&event_type_hash, &event_id, Source::Output)?;
        if event_in.len()!=EVENT_DATA_LEN || event_out.len()!=EVENT_DATA_LEN { return Err(ERR); }
        if event_in[EVENT_OFFSET_STATUS] != STATUS_FINISHED || event_out[EVENT_OFFSET_STATUS] != STATUS_SETTLED { return Err(ERR); }
        if bytes::<32>(&event_out, EVENT_OFFSET_TREASURY_LOCK)? != my_lock_hash { return Err(ERR); }
        let (ic, in_pot) = spendable_pot(&my_lock_hash, Source::Input)?;
        let (oc, _) = spendable_pot(&my_lock_hash, Source::Output)?;
        if ic != 1 || oc != 0 { return Err(ERR); }
        if read_u64(&event_in, EVENT_OFFSET_POT)? != in_pot { return Err(ERR); }
        if read_u64(&event_out, EVENT_OFFSET_FINAL_POT)? != in_pot || read_u64(&event_out, EVENT_OFFSET_POT)? != 0 { return Err(ERR); }
...
fn validate_claim_outputs(event:&[u8], event_type_hash:&[u8;32], final_pot:u64)->Result<(),i8>{
    ...
    for i in 0..n {
        ...
        // Match by code_hash; claim args already bind event_type_hash || event_id.
        ...
        // Reward-claim-type binds the output lock to recipient, so treasury only checks the payout set.
        ...
        if found>winners_n{return Err(ERR);}
    }
    if final_pot==0 {return Ok(());}
    if found==0 || sum!=final_pot{return Err(ERR);}
    Ok(())
}
