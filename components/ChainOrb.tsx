...
export function ChainOrb({ state, cellHash }: ChainOrbProps) {
  ...
  return (
    <div className="relative flex h-[260px] w-[260px] items-center justify-center" aria-hidden="true">
      ...
      <motion.div
        className="relative flex h-40 w-40 items-center justify-center border-[6px] border-black bg-[#224cff] shadow-[10px_10px_0_#101010]"
        animate={dead ? {} : { rotate: [0, -3, 2, 0] }}
        transition={{ duration: state === 'critical' ? 0.55 : 2.7, repeat: Infinity }}
      >
        <div
          className="flex h-28 w-28 items-center justify-center border-[5px] border-black"
          style={{ backgroundColor: color.face }}
        >
          {dead ? (
            <Lock className="h-9 w-9 stroke-[3] text-black" />
          ) : (
            <span className="font-poster text-5xl leading-none text-black">C</span>
          )}
        </div>
