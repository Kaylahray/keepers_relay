...
export function ArtifactStudio({
  ...
  const cover = resolveCover(coverImageUrl, creatureName || artifact.title);
    ...
    <section aria-labelledby="artifact-title" className="neo-card overflow-hidden bg-[#ffe454] p-0 text-black">
      <div className="grid md:grid-cols-[0.92fr_1.08fr]">
        ...
        <div className="p-5 sm:p-6">
          ...
          {isKeeper && !hasContributedThisTurn && (
            <p className="mt-4 border-2 border-black bg-[#ff4cbd] p-2.5 text-xs font-bold">
              Leave your mark, then pass.
            </p>
          )}
          {isKeeper && hasContributedThisTurn && (
            <p className="mt-4 border-2 border-black bg-[#d6ff00] p-2.5 text-xs font-bold">
              Mark sealed. Pass when you’re ready.
            </p>
          )}
          {featured && (
          ...
          {isKeeper ? (
            <form onSubmit={submit} className="mt-5 border-t-[3px] border-black pt-4">
              ...
              <input
                ...
                className="mt-2 w-full border-[3px] border-black bg-[#fff8e7] px-3 py-2 text-sm font-semibold outline-none focus:bg-white"
              />
              <div className="mt-3 flex items-center justify-between gap-3">
                <span className="font-mono text-[10px] font-bold">{body.length}/180 · FOREVER</span>
                <button
                  type="submit"
                  disabled={!body.trim() || publishing || hasContributedThisTurn}
      ...
      <div className="border-t-[3px] border-black bg-black px-5 py-4 text-[#fff8e7]">
        <div className="flex items-center justify-between gap-3">
          ...
          <Link
            ...
            className="flex items-center gap-1 border-2 border-[#fff8e7] px-2 py-1 text-[10px] font-black uppercase"
          >
            Full archive
